...
public class Main {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      ArrayList<CompletableFuture<BusRoutes>> allFinal = new ArrayList<>();
      System.gc();
      while (sc.hasNext()) {
        BusStop srcId = new BusStop(sc.next());
        String searchString = sc.next();
        System.gc();
        allFinal.add(BusSg.findBusServicesBetween(srcId, searchString));
      }
      CompletableFuture<?>[] arrayDone = allFinal.toArray(new CompletableFuture[allFinal.size()]);

      CompletableFuture<Void> done = CompletableFuture.allOf(arrayDone);
      done.join();
      for (CompletableFuture<BusRoutes> a : allFinal) {
        System.gc();
        System.out.println(a.thenComposeAsync(y -> y.description()).join());
      }
      sc.close();
